...
console.log(
  chalk.blue(`${icons.start} Starting APP in "${env}" environment...`),
);
...
try {
  ...
  console.log(
    chalk.cyan(
      `${icons.building} Running start command for "${env}" environment ...`,
    ),
  );
  execSync(command, { stdio: 'inherit' });
  console.log(
    chalk.green(
      `${icons.success} APP started successfully in "${env}" environment.`,
    ),
  );
} catch (error) {
  console.error(
    chalk.red(
      `${icons.error} An error occurred while starting the APP in "${env}":`,
      error.message,
    ),
  );
  process.exit(1);
